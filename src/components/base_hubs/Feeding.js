...
function Feeding (props){
  ...
  const getFeed = feed.filter(hub => hub.id === 1)
    ...
    <section>
      <div className="container">
        ...
        <div className="row multi-row-clearfix">
          ...
          {currentPosts.map(feedPro => (        


<div className="col-md-4" key ={feed.id}>
...
/>  
    ...
    <ul className="list-inline font-18 font-weight-600 clearfix mb-5">
      <li className="pull-left font-weight-400 text-black-333 pr-0">{t('Raised:')} <span className="text-theme-colored font-weight-700">{feedPro.id} SDG</span></li>
      <li className="pull-right font-weight-400 text-black-333 pr-0">{t('Goal:')}<span className="text-theme-colored font-weight-700">{feedPro.id} SDG</span></li>
    </ul>
      <h4 className="text-uppercase">{feedPro.title}</h4>
    <div className="progress-item mt-0">
      <div className="progress mb-0">
        <div data-percent={feedPro.id} className="progress-bar"><span className="percent">0</span></div>
      </div>
    </div>
    <p className="mt-20">{feedPro.description}.</p>
    <Link to={hub_name+'/project/'+feedPro.id} className="btn btn-default btn-theme-colored btn-xs font-16 mt-10">Donate</Link>
  </div>
</div>
</div>
))}
